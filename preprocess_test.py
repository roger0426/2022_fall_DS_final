...
# product_list = ['ind_ahor_fin_ult1', 'ind_aval_fin_ult1', 'ind_cco_fin_ult1', 'ind_cder_fin_ult1', 
...
# history_list = ['ind_ahor_fin_ult1_history', 
# 'ind_aval_fin_ult1_history',  'ind_cco_fin_ult1_history',
# 'ind_cder_fin_ult1_history',  'ind_cno_fin_ult1_history',
# 'ind_ctju_fin_ult1_history',  'ind_ctma_fin_ult1_history',
# 'ind_ctop_fin_ult1_history',  'ind_ctpp_fin_ult1_history',
# 'ind_deco_fin_ult1_history',  'ind_deme_fin_ult1_history',
# 'ind_dela_fin_ult1_history',  'ind_ecue_fin_ult1_history',
# 'ind_fond_fin_ult1_history',  'ind_hip_fin_ult1_history',
# 'ind_plan_fin_ult1_history',  'ind_pres_fin_ult1_history',
# 'ind_reca_fin_ult1_history',  'ind_tjcr_fin_ult1_history',
# 'ind_valo_fin_ult1_history',  'ind_viv_fin_ult1_history',
# 'ind_nomina_ult1_history', 'ind_nom_pens_ult1_history',
# 'ind_recibo_ult1_history']

# to_bool_list = product_list + history_list
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_data', type=Path, default=Path('./data/test_ver2.csv.zip'))
    parser.add_argument('--train_data_path', type=Path, default=Path('./data/train_preprocessed_v4.pkl'))
    parser.add_argument('--output_data', type=Path, default=Path('./data/test.pkl'))

    parser.add_argument('--canal_entrada_list_data', type=Path, default=Path('./data/customer_info_transfer_list.txt'))
    ...
    try:
        canal_entrada_list = list(pd.read_csv(args.canal_entrada_list_data).columns)
    except:
        logging.info('===> canal_entrada_list not found !!')
        canal_entrada_list = None
    print(canal_entrada_list)
    ...
    picked_col = [item for item in train_data.columns if '_3month' not in item]
    # print(picked_col, '\n')

    product_col = [item for item in picked_col if ('ind_' in item and '_ult1' in item)]
    to_bool_list = product_col
    # print(product_col, '\n')

    history_list = [item for item in picked_col if ('ind_' in item and '_ult1_history' in item)]
    # print(history_list, '\n')


    # TODO: concate via "cnodpers"
    # print(data, concate_data)
    # print(concate_data.loc[concate_data['fecha_alta']!=concate_data['fecha_alta'].max()])
    data.set_index('ncodpers')
    concate_data = train_data[['ncodpers'] + product_col]
    # print(concate_data)
    print(data)
    concate_data = concate_data.groupby('ncodpers').mean()#.set_index('ncodpers')
    for target in history_list: 
        concate_data[target] = (concate_data[target] + concate_data[target.replace('_history', '')]).astype(bool)
    print(concate_data)
    data = pd.concat([data.set_index('ncodpers'), concate_data], axis=1, join='outer')
    data.reset_index()
    print(data)
    ...
    if canal_entrada_list == None:
        canal_entrada_others = [canel for canel in data['canal_entrada'].unique() if (data['canal_entrada'].value_counts()[canel] < len(data)/1000)]
    else:
        canal_entrada_others = [canel for canel in data['canal_entrada'].unique() if canel not in canal_entrada_list]
        print(canal_entrada_list, canal_entrada_others)
